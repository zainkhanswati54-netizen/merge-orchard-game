...
import { trackMissionProgress as trackActiveMission, getActiveMission } from './missionSelect.js';
import { JuiceEngine } from './juiceEngine.js';
import { startMusicEngine, stopMusicEngine, setMusicVolume, setMusicEnergy } from './musicEngine.js';
...
import { usePower, POWER_DEFS } from './powers.js';
import { checkAndStartBossForCurrentLevel, hitBoss, isBossActive, onBossEvent, getActiveBoss } from './bossManager.js';
import { checkNewlyUnlockedSkills, getSkillModifiers, markPhoenixUsed } from './skillTree.js';
...
export class Game {
  constructor(canvas, theme, weatherOverride = null) {
    // Preload orchard background image
    if (!Game._bgImage) {
      Game._bgImage = new Image();
      Game._bgImage.src = 'assets/orchard_bg.svg';
    }
    this.canvas = canvas;
    ...
    this.juice = new JuiceEngine();
    this.juice.onScreenShake((amt, ms) => this.shake.trigger(amt, ms));
    this.shake = new ScreenShake();
    this.weather = new WeatherSystem();
    this.ui = new UI({
      onRestart: () => this.restart(),
      onUsePower: (id) => this._activatePower(id),
      onPhoenixContinue: () => { this.paused = false; },
    });
    this.input = new InputController(canvas, {
      ...
      onFirstInteraction: () => {
        preloadAudio();
        startMusicEngine(0.35);
    setMusicEnergy(0.2); // start calm, escalate with gameplay
      },
    });
    ...
    // Phoenix fruit (skill tree) — lifetime-once across the whole save,
    // not refilled per game. getSkillModifiers() already factors in
    ...
    onAchievementUnlocked((ach) => {
      const cx = (this.physics.playLeft + this.physics.playRight) / 2;
      this.floatingText.spawnCallout(cx, CONFIG.BOX_HEIGHT * 0.25, `${ach.name.toUpperCase()}!`, '#FFC857');
  ...
  _checkNewUnlocks(beforeXP, afterXP) {
    ...
    for (const theme of THEMES) {
      ...
      if (wasLocked && nowUnlocked) {
        this.juice.onUnlock(cx, CONFIG.BOX_HEIGHT * 0.3);
    ...
    if (newLevel > beforeInfo.level) {
      const zoneMessages = {
        ...
        25: ['The Sky Orchard rises!', '#4DD0E1'],
    ...
    for (const skill of newSkills) {
      setTimeout(() => {
        this.floatingText.spawnCallout(cx, CONFIG.BOX_HEIGHT * 0.18, `${skill.name.toUpperCase()} UNLOCKED!`, '#A8E6CF');
        playUnlockSound();
  ...
  _resetState() {
    ...
    this.proximityTension = 0; // 0..1 ambient "getting close" feeling, separate from the settle-timer
    try {
  ...
  tryDrop() {
    ...
    this.juice.onDrop(this.dropperX, CONFIG.DROPPER_Y);
    this.shake.trigger(CONFIG.SCREEN_SHAKE_DROP, 120);

    this._sessionDrops++;
    recordDrop();
    reportMissionProgress('drop', 1);
    trackActiveMission('drop', 1);
  ...
  _resolveMerge(a, b, tier) {
    ...
    this.juice.onMerge(mx, my, tier, color);
    ...
    if (coinGain >= 3) this.juice.onCoinEarned(mx, my);
    if (coinGain > 0) trackActiveMission('coins', coinGain);
    ...
    this._sessionMerges++;
    recordMerge();
    reportMissionProgress('merge', 1);
    reportMissionProgress('score', scoreGain);
    // Active mission tracking
    const _mResult = trackActiveMission('merge', 1)
      || trackActiveMission('score', scoreGain)
      || trackActiveMission('bigMerge', 1, { tier });
    // Update in-game mission reminder bar
    const _activeMis = getActiveMission();
    if (_activeMis) {
      const { getMissionProgress: _getMP } = window.__missionMod || {};
      const _prog = _getMP?.(_activeMis.id) || 0;
      import('./missionSelectUI.js').then(m => m.updateIngameMissionBar(_activeMis, _prog));
      if (_mResult === 'completed') {
        import('./missionSelectUI.js').then(m => m.showMissionCompleteToast(_activeMis));
        import('./missionSelect.js').then(m => m.setActiveMission(null));
      }
    }
    ...
    if (isBossActive()) {
      ...
      if (bossResult?.defeated) {
        ...
        this.floatingText.spawnCallout(mx, my - 60, `+${bossResult.boss.reward.coins} Coins  +${bossResult.boss.reward.xp} XP`, '#FFD700');
      } else if (bossResult) {
    ...
    if (this.comboCount > this._sessionBestCombo) {
      ...
      reportMissionProgress('combo', this.comboCount);
      trackActiveMission('combo', this.comboCount);
    }
    ...
    if (this.comboCount >= 2) {
      ...
      this.juice.onCombo(mx, my, this.comboCount);
      setMusicEnergy(Math.min(1, 0.5 + this.comboCount * 0.06)); // music pumps on combo
    ...
    if (xpResult.leveledUp) {
      ...
      this.juice.onLevelUp(CONFIG.BOX_WIDTH / 2, CONFIG.BOX_HEIGHT / 2);
  ...
  _activatePower(id) {
    ...
    if (id === 'bomb') {
      // Remove the top N highest fruits (lowest Y value = highest in jar).
      // N comes from POWER_DEFS.clearCount (2, down from the original 4) —
      // bomb still helps in a pinch but no longer single-handedly erases
      // a whole near-loss stack.
      const clearCount = def?.clearCount ?? 2;
      const bodies = [...this.physics.getItemBodies()].sort((a, b) => a.position.y - b.position.y);
      const toRemove = bodies.slice(0, Math.min(clearCount, bodies.length));
      ...
      this.juice.onPowerBomb(cx, CONFIG.BOX_HEIGHT * 0.4);
    }

    else if (id === 'magnet') {
      const skillMods = getSkillModifiers();
      const baseDuration = def?.durationMs ?? CONFIG.MAGNET_DURATION_MS;
      const bonusDuration = skillMods.hasMagnetPlus ? 2000 : 0;
      this._magnetUntil = now + baseDuration + bonusDuration;
      this.floatingText.spawnCallout(cx, CONFIG.BOX_HEIGHT * 0.4, 'MAGNET!', '#7C4DFF');
      this.juice.onPowerMagnet(cx, CONFIG.BOX_HEIGHT * 0.4);
    }
      ...
      if (merged) {
        ...
        this.juice.onPowerLightning(cx, 0, cx, CONFIG.BOX_HEIGHT * 0.6);
      }
    }
      ...
      this.juice.onPowerRainbow(cx, CONFIG.BOX_HEIGHT * 0.4);
    }
  ...
  _applyMagnet() {
    const def = POWER_DEFS.find((p) => p.id === 'magnet');
    const skillMods = getSkillModifiers();
    const baseForce = def?.pullForce ?? 0.00035;
    const force = skillMods.hasMagnetPlus ? baseForce * 1.4 : baseForce;
    const bodies = [...this.physics.getItemBodies()];
    const byTier = new Map();
    for (const b of bodies) {
      if (!byTier.has(b.tier)) byTier.set(b.tier, []);
  ...
  // Ambient "getting close" feedback — separate from _updateDangerTimer.
  // _updateDangerTimer only reacts AFTER a fruit has already settled on
  // the danger line, which means the player gets zero warning before that
  // moment. This builds a 0..1 tension value purely from how close the
  // top of the stack is to the danger line, so the glow/heartbeat starts
  // ramping up while the stack is still rising — real advance notice,
  // like seeing a train coming rather than only reacting to the crash.
  // Combined with the settle-timer's "progress" in _updateDangerTimer,
  // this becomes one continuous escalation: building unease as the stack
  // climbs, sharpening into a fast, urgent heartbeat once something is
  // actually parked on the line and the clock is running out.
  _updateProximityTension(dtMs) {
    const target = clamp(
      (CONFIG.DANGER_LINE_Y + CONFIG.APPROACH_ZONE_Y - this.currentPeakY) / CONFIG.APPROACH_ZONE_Y,
      0, 1
    );
    // Smooth toward the target so it doesn't flicker with every tiny physics jiggle.
    this.proximityTension = lerp(this.proximityTension, target, Math.min(1, dtMs / 220));
  }
  ...
  _updateDangerTimer(dtMs) {
    ...
    const settleProgress = clamp(this.dangerTimer / CONFIG.DANGER_TIME_LIMIT_MS, 0, 1);
    this.ui.setDangerProgress(settleProgress);

    // Combined tension: whichever signal is stronger wins. Below the line,
    // proximityTension carries it; once something settles on the line,
    // settleProgress (which can reach a harder-pulsing 1.0) takes over.
    const combined = Math.max(this.proximityTension * 0.6, settleProgress);
    this.ui.setApproachGlow(this.proximityTension);

    if (combined > 0.04) {
      const interval = lerp(CONFIG.HEARTBEAT_INTERVAL_MAX_MS, CONFIG.HEARTBEAT_INTERVAL_MIN_MS, combined);
      if (now - this.lastHeartbeatTime >= interval) {
        this.lastHeartbeatTime = now;
        this.juice.onDanger(combined);
      }
    } else {
      this.lastHeartbeatTime = 0;
    }
  ...
  _triggerGameOver() {
    // Phoenix Fruit: lifetime-once save. Once this fires, markPhoenixUsed()
    // permanently disables it (across all future games too) — it is not
    // refilled by starting a new run.
    if (this._phoenixAvailable) {
      ...
      // Pause and show a full, must-acknowledge overlay rather than a
      // small fading bit of canvas text — this moment is rare (it can
      // only ever happen once, ever) and should be unmistakable, never
      // confusable with a silent restart.
      this.paused = true;
      this.ui.showPhoenixSave();
      return; // Don't actually game over
    }

    this.state = 'gameover';
    this.juice.onGameOver(CONFIG.BOX_WIDTH / 2, CONFIG.BOX_HEIGHT / 2);
  ...
  _loop(now) {
    ...
    if (this.state === 'playing' && !this.paused) {
      ...
      // Also rescue any stray bodies that escape the play area
      for (const b of this.physics.getItemBodies()) {
        ...
        // Stray body cleanup: ball fell through floor — snap back above ground
        const r = b.circleRadius;
        if (b.position.y > CONFIG.BOX_HEIGHT + r * 3) {
          Matter.Body.setPosition(b, { x: b.position.x, y: CONFIG.BOX_HEIGHT - r - 2 });
          Matter.Body.setVelocity(b, { x: 0, y: 0 });
        }
        // Ball escaped sideways — bring back to center
        if (b.position.x < -r * 2 || b.position.x > CONFIG.BOX_WIDTH + r * 2) {
          Matter.Body.setPosition(b, { x: CONFIG.BOX_WIDTH / 2, y: b.position.y });
          Matter.Body.setVelocity(b, { x: 0, y: b.velocity.y });
        }
      }

      this._updateProximityTension(dtMs);
  ...
  _render(now) {
    ...
    this._drawJar(ctx);
    this.juice.drawUnder(ctx, CONFIG.BOX_WIDTH, CONFIG.BOX_HEIGHT);
  ...
  _drawDangerPulse(ctx, now) {
    ...
    const approach = this.proximityTension || 0;
    // Combined visual intensity: proximity alone tops out at a noticeable-
    // but-not-alarming glow; once something actually settles on the line
    // (dangerProgress > 0) the effect sharpens toward full intensity.
    const combinedLevel = Math.max(approach * 0.55, dangerProgress);
    if (combinedLevel <= 0.01) return;
    ...
    // Below ~0.5s/beat even at low tension, so the very first pulses are
    // calm — speeding up is itself part of the warning.
    const interval = lerp(CONFIG.HEARTBEAT_INTERVAL_MAX_MS, CONFIG.HEARTBEAT_INTERVAL_MIN_MS, combinedLevel);
    const pulse = 0.5 + 0.5 * Math.sin((now / interval) * Math.PI * 2);
    const intensity = combinedLevel * pulse;
    ...
    const glowW = 24 + combinedLevel * 10;
    ...
    ctx.lineWidth = 1.5 + combinedLevel * 1.5;
    ctx.strokeStyle = `rgba(255,50,50,${lineAlpha.toFixed(3)})`;
    ctx.shadowBlur  = 6 + combinedLevel * 12;
    ctx.shadowColor = 'rgba(255,30,30,0.9)';
    ctx.stroke();
    ctx.shadowBlur  = 0;

    // ── "CRITICAL" text once a fruit is actually settled on the line ─────
    // This is the moment the player most needs an unambiguous signal that
    // the clock is genuinely running, distinct from the earlier ambient
    // build-up — text only appears in the sharp, post-settle phase.
    if (dangerProgress > 0.15) {
      ctx.save();
      ctx.globalAlpha = 0.5 + 0.5 * pulse;
      ctx.font = '900 13px var(--font-display), sans-serif';
      ctx.textAlign = 'center';
      ctx.fillStyle = '#FF3B3B';
      ctx.shadowColor = 'rgba(255,30,30,0.9)';
      ctx.shadowBlur = 8;
      ctx.fillText('CRITICAL', (left + right) / 2, CONFIG.DANGER_LINE_Y - 8);
      ctx.restore();
    }
  ...
  _drawJar(ctx) {
    ...
    // Draw orchard background image inside the play area
    const bgImg = Game._bgImage;
    if (bgImg && bgImg.complete && bgImg.naturalWidth > 0) {
      ctx.save();
      roundRectPath(ctx, left + 2, 2, width - 4, CONFIG.BOX_HEIGHT - 4, 18);
      ctx.clip();
      // Draw image scaled to fill play area
      ctx.globalAlpha = 0.55; // semi-transparent so fruits stand out
      ctx.drawImage(bgImg, left + 2, 2, width - 4, CONFIG.BOX_HEIGHT - 4);
      // Dark gradient overlay on top for readability
      const overlay = ctx.createLinearGradient(0, 0, 0, CONFIG.BOX_HEIGHT);
      overlay.addColorStop(0, 'rgba(0,0,0,0.25)');
      overlay.addColorStop(0.5, 'rgba(0,0,0,0.05)');
      overlay.addColorStop(1, 'rgba(0,0,0,0.3)');
      ctx.globalAlpha = 1;
      ctx.fillStyle = overlay;
      ctx.fillRect(left + 2, 2, width - 4, CONFIG.BOX_HEIGHT - 4);
      ctx.restore();
    } else {
      const bgGrad = ctx.createLinearGradient(0, 0, 0, CONFIG.BOX_HEIGHT);
      bgGrad.addColorStop(0, 'rgba(20,60,20,0.9)');
      bgGrad.addColorStop(1, 'rgba(10,30,10,0.95)');
