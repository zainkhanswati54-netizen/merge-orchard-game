...
export class UI {
  ...
  showGameOver(finalScore, sessionStats = {}) {
    ...
    // Update power button labels to show energy cost
  ...
  updatePowerButtons() {
    ...
    for (const def of POWER_DEFS) {
      ...
      btn.title = `${def.name} — Cost: ${energyCost} ✨ Life Energy\n${def.desc}`;
      btn.innerHTML = `
        <span class="power-emoji">${def.emoji}</span>
        <span class="power-count">${count}</span>
        <span class="power-energy-cost">✨${energyCost}</span>
      `;
  ...
  // Update the Orchard status HUD (Life Energy, Health, Blight)
  setOrchardMeters(energy, health, blight) {
    // Life Energy bar
    const energyFill = document.getElementById('energy-bar-fill');
    const energyVal = document.getElementById('energy-bar-value');
    if (energyFill) energyFill.style.width = `${Math.min(100, (energy / 200) * 100)}%`;
    if (energyVal) energyVal.textContent = Math.floor(energy);

    // Orchard Health bar
    const healthFill = document.getElementById('orchard-health-fill');
    const healthVal = document.getElementById('orchard-health-value');
    if (healthFill) {
      healthFill.style.width = `${health}%`;
      healthFill.style.background = health > 60 ? '#4CAF50' : health > 30 ? '#FF9800' : '#F44336';
    }
    if (healthVal) healthVal.textContent = Math.floor(health);

    // Blight meter
    const blightFill = document.getElementById('blight-fill');
    const blightVal = document.getElementById('blight-value');
    const blightMeter = document.getElementById('blight-meter');
    if (blightMeter) blightMeter.style.display = blight > 0 ? '' : 'none';
    if (blightFill) {
      blightFill.style.width = `${blight}%`;
      blightFill.classList.toggle('blight-critical', blight > 70);
    }
    if (blightVal) blightVal.textContent = Math.floor(blight);
  }
}
