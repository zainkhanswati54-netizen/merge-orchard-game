...
import { startMusicEngine, setMusicEnergy } from './musicEngine.js';
...
import { POWER_DEFS, getPowerCount, buyPower, isPowerUnlocked } from './powers.js';
...
const SCREEN_ELEMENT_ID = {
  ...
  'mission-select-screen': 'mission-select-screen',
  game:                    'game-screen',
};

// Orchard zones — shown in the restoration bar based on XP level
const ORCHARD_ZONES = [
  { minLevel: 1,  label: 'The Forgotten Field' },
  { minLevel: 5,  label: 'The Apple Grove' },
  { minLevel: 10, label: 'Cherry Valley' },
  { minLevel: 15, label: 'The Golden Orchard' },
  { minLevel: 20, label: 'The Crystal Forest' },
  { minLevel: 25, label: 'The Sky Orchard' },
  { minLevel: 30, label: 'The Eternal Orchard' },
];
...
export function showScreen(name) {
  ...
  if (!targetId) { console.warn('[screens] Unknown screen:', name); return; }

  // Hide ALL screens first, then show only target
  ...
  if (name === 'menu') {
    ...
    _updateMenuFruits();
    try { startMusicEngine(0.22); setMusicEnergy(0.12); } catch(e) {}
  }
}
...
function _updateOrchardRestoration() {
  ...
  if (zoneEl) {
    ...
    zoneEl.textContent = currentZone.label;
  }
}
...
function _updateStreakDisplay() {
  ...
  el.innerHTML = `<span class="streak-count">${streak}-day streak</span>${multLabel}`;
}
...
function renderWeatherPicker() {
  ...
  for (const opt of WEATHER_OPTS) {
    ...
    btn.title = opt.label;
    btn.innerHTML = `<span class="weather-opt-label">${opt.label}</span>`;
  ...
  for (const m of missions) {
    ...
    li.innerHTML = `
      ...
      <div class="mission-reward">
        ${m.reward.coins} Coins · ${m.reward.xp} XP
...
function renderPowerShop() {
  ...
  for (const def of POWER_DEFS) {
    ...
    const unlocked = isPowerUnlocked(def.id);
    const canAfford = unlocked && coins >= def.cost;
    const div = document.createElement('div');
    ...
    if (!unlocked) {
      div.innerHTML = `
        <div class="shop-info">
          ...
          <div class="shop-locked-note">Unlocks at Level ${def.unlockLevel} (you're Lv ${level})</div>
        </div>
        <button class="shop-buy-btn shop-btn-disabled" disabled>
          Lv ${def.unlockLevel}
        </button>
      `;
    } else {
  ...
  listEl.querySelectorAll('[data-buy-power]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (success) {
        playClickSound();
        renderPowerShop();
        document.getElementById('shop-coins-display').textContent = `Coins: ${getTotalCoins()}`;
      } else {
...
export function initScreens({ onSelectLevel, onPauseGame, onResumeGame, onRestartGame, onPlayBtn } = {}) {
  ...
  document.getElementById('menu-shop-btn')?.addEventListener('click', () => {
    ...
    if (el) el.textContent = `Coins: ${getTotalCoins()}`;
    renderPowerShop();
  ...
  document.getElementById('reset-game-btn')?.addEventListener('click', () => {
    ...
    if (confirmed) {
      // Clear all localStorage keys related to game
      const keysToDelete = [];
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key && (key.startsWith('suika_') || key.startsWith('legend_') || key.startsWith('merge_'))) {
  ...
  if (musicSlider) {
    ...
    musicSlider.addEventListener('input', () => {
      ...
      try { const { setMusicVolume } = window.__musicEngine || {}; setMusicVolume?.(vol); } catch(e){}
    });
  }
  ...
  document.querySelectorAll('.track-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      try {
        const { setMusicTrack } = window.__musicEngine || {};
        setMusicTrack?.(track);
        // Save preference
        try { localStorage.setItem('music_track', track); } catch(e) {}
      } catch(e) {}
    });
  });

  // UI theme picker
  const savedTheme = (() => { try { return localStorage.getItem('ui_theme') || 'dark-forest'; } catch(e) { return 'dark-forest'; } })();
  ...
  document.querySelectorAll('.ui-theme-btn').forEach(btn => {
    ...
    btn.addEventListener('click', () => {
      document.querySelectorAll('.ui-theme-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
...
function _initStoryScreen(onComplete) {
  ...
  // Clone buttons to remove any old duplicate listeners
  function reclone(el) {
    if (!el) return null;
  ...
  const freshPrev = reclone(prevBtn);
  const freshNext = reclone(nextBtn);
  const freshSkip = reclone(skipBtn);
...
export async function runLoadingSequence() {
  ...
  setProgress(5,  'Awakening the orchard…');
  await new Promise(r => setTimeout(r, 80));
  setProgress(30, 'Calling the guardians…');
  await new Promise(r => setTimeout(r, 80));
  setProgress(60, 'Loading fruits…');
  await new Promise(r => setTimeout(r, 80));
  ...
  // Show story on very first launch, then always go to menu
  let storySeen = false;
  ...
  if (!storySeen) {
    _initStoryScreen(() => {
      ...
      // Show daily gift after story if needed
      setTimeout(() => showDailyLoginIfNeeded(), 800);
    });
    showScreen('story');
  } else {
    showScreen('menu');
...
function renderWeeklyMissions() {
  ...
  for (const m of missions) {
    ...
    div.innerHTML = `
      ...
      <div class="mission-bar-track"><div class="mission-bar-fill" style="width:${pct}%"></div></div>
      <div class="mission-reward">
        ${m.reward.coins.toLocaleString()} Coins · ${m.reward.xp.toLocaleString()} XP
        ${m.completed && !m.rewardClaimed
      ...
      <div class="ach-info">
        ...
        ${isAccessible ? `
          <div class="ach-desc" style="color:#FF6B35;margin-top:3px;">
            ${defeated
              ? `Awakened! Reward: ${boss.reward.coins} Coins · ${boss.reward.xp} XP`
              : `Needs ${boss.hpRequired} merges to awaken · Reward: ${boss.reward.coins} Coins · ${boss.reward.xp} XP`}
          </div>` : ''}
      </div>
    `;
    listEl.appendChild(div);
...
// ---------------------------------------------------------------------------
// MINI MISSION TRACKER — shown in HUD during gameplay
// ---------------------------------------------------------------------------
export function updateHudMissions() {
  const listEl = document.getElementById('hud-missions-list');
  if (!listEl) return;
  const missions = getDailyMissions();
  listEl.innerHTML = '';
  for (const m of missions) {
    ...
    const div = document.createElement('div');
    div.className = `mission-mini-item${m.completed ? ' done' : ''}`;
    div.innerHTML = `
      <div class="mission-mini-desc">${m.desc}</div>
      <div class="mission-mini-bar-track">
        <div class="mission-mini-bar-fill" style="width:${pct}%"></div>
      </div>
    `;
    listEl.appendChild(div);
  }
...
// ---------------------------------------------------------------------------
// MENU FRUITS — swap emoji based on UI theme
// ---------------------------------------------------------------------------
function _updateMenuFruits() {
  const theme = document.documentElement.getAttribute('data-ui-theme') || 'dark-forest';
  const THEME_FRUITS = {
    'dark-forest': ['🍎', '🍊', '🍋', '🍇', '🍓', '🍉'],
    'golden-dawn': ['🍑', '🍊', '🌽', '🍋', '🥭', '🍯'],
    'ocean-deep':  ['🫐', '🥝', '🫒', '🍈', '🥑', '🍃'],
  ...
  const fruits = THEME_FRUITS[theme] || THEME_FRUITS['dark-forest'];
  document.querySelectorAll('.menu-fruit').forEach((el, i) => {
...
// ---------------------------------------------------------------------------
// CHAPTER INFO POPUP — shown when player taps Ch.X in HUD
// ---------------------------------------------------------------------------
export function initChapterPopup(getChapterFn, getMissionsFn) {
  const popup  = document.getElementById('chapter-popup');
  const closeBtn = document.getElementById('chapter-popup-close');
  const triggerEl = document.getElementById('chapter-info-bar');

  if (!popup || !triggerEl) return;

  function openPopup() {
    const chapter  = getChapterFn();
    const missions = getMissionsFn();

    // Title + area
    const el = (id) => document.getElementById(id);
    if (el('cp-title'))  el('cp-title').textContent  = `Chapter ${chapter.id}`;
    if (el('cp-area'))   el('cp-area').textContent   = chapter.areaName;

    // Stars
    const stars = chapter._earnedStars || 0;
    for (let i = 1; i <= 3; i++) {
      const s = el(`cp-star-${i}`);
      if (s) { s.textContent = i <= stars ? '★' : '☆'; s.classList.toggle('earned', i <= stars); }
    }

    // Star targets
    const t = chapter.starsTarget || [350, 700, 1200];
    if (el('cp-t1')) el('cp-t1').textContent = t[0]?.toLocaleString() + ' pts';
    if (el('cp-t2')) el('cp-t2').textContent = t[1]?.toLocaleString() + ' pts';
    if (el('cp-t3')) el('cp-t3').textContent = t[2]?.toLocaleString() + ' pts';

    // Missions
    const mBox = el('cp-missions');
    if (mBox && missions) {
      mBox.innerHTML = '';
      for (const m of missions) {
        const pct = Math.min(100, Math.round((m.progress / m.target) * 100));
        const div = document.createElement('div');
        div.className = `cp-mission-item${m.completed ? ' done' : ''}`;
        div.innerHTML = `
          <div class="cp-mission-label">${m.desc}</div>
          <div class="cp-mission-progress">${m.progress} / ${m.target}</div>
          <div class="cp-mission-bar"><div class="cp-mission-bar-fill" style="width:${pct}%"></div></div>
        `;
    ...
    // Rewards
    const rBox = el('cp-rewards');
    if (rBox && chapter.reward) {
      rBox.innerHTML = `
        <div class="cp-reward-pill">🪙 ${chapter.reward.coins} Coins</div>
        <div class="cp-reward-pill">✨ ${chapter.reward.xp} XP</div>
      `;
    }
  ...
  // Re-render popup if it's open when missions change
  return () => { if (popup.style.display !== 'none') openPopup(); };
}
...
function _renderChapterGrid(areaId) {
  ...
  for (const ch of chapters) {
    ...
    const modInfo = ch.modifier === 'guardian_trial' ? `<span class="chapter-cell-modifier">Trial</span>` : '';

    cell.innerHTML = `
      ${modInfo}
      ${isBoss ? `<span class="chapter-cell-boss-icon">Boss</span>` : `<span class="chapter-cell-num">${ch.id}</span>`}
...
export function updateChapterBadge() {
  ...
  if (el) el.textContent = `Chapter ${progress.completedChapters + 1} of 50`;
  if (starsEl) starsEl.textContent = `★ ${progress.totalStars}/450`;

  // Count badge
  const countBadge = document.getElementById('chapter-count-badge');
  if (countBadge) countBadge.textContent = `${progress.completedChapters}/50`;
}
...
export function initDailyLogin(onClaim) {
  ...
  const oldBtn = document.getElementById('daily-claim-btn');
  if (oldBtn) {
    const claimBtn = oldBtn.cloneNode(true);
    oldBtn.parentNode.replaceChild(claimBtn, oldBtn);

    claimBtn.addEventListener('click', () => {
      playClickSound();
      const reward = claimLoginReward();
      if (reward) onClaim?.(reward);
      // Always close — even if reward was null/already claimed
...
function _renderDailyLoginModal(streakDay) {
  ...
  if (calEl) {
    ...
    for (let d = 1; d <= 7; d++) {
      ...
      const status = d <= streakDay ? 'Done' : '';
      dayEl.innerHTML = `<span class="daily-cal-day-icon">${status}</span><span class="daily-cal-day-label">Day ${d}</span>`;
...
export function renderOrchardTasks() {
  ...
  for (const task of tasks) {
    ...
    div.innerHTML = `
      ...
      <div class="mission-bar-track"><div class="mission-bar-fill" style="width:${pct}%"></div></div>
      <div class="mission-reward">
        ${task.reward.coins} Coins · ${task.reward.xp} XP
...
// ---------------------------------------------------------------------------
// MISSION SELECT SCREEN — init and render
// ---------------------------------------------------------------------------
let _missionSelectCallback = null;
let _selectedMission = null;

export function initMissionSelect(onPlay) {
  _missionSelectCallback = onPlay;

  document.getElementById('ms-back-btn')?.addEventListener('click', () => {
    showScreen('menu');
  });

  document.getElementById('ms-play-btn')?.addEventListener('click', () => {
    if (_selectedMission) {
      _missionSelectCallback?.(_selectedMission, false);
    }
  });

  document.getElementById('ms-freeplay-btn')?.addEventListener('click', () => {
    _missionSelectCallback?.(null, true);
  });
}

export function showMissionSelect(chapter, levelInfo) {
  // Import needed modules
  import('./missionSelect.js').then(({ getChapterMissions, getMissionProgress, isMissionCompleted, getDifficultyColor, getChapterMissionStats }) => {
    _selectedMission = null;

    // Update header
    document.getElementById('ms-chapter-name').textContent = chapter.name;
    document.getElementById('ms-area-name').textContent   = chapter.areaName;

    // Stars badge
    const stats = getChapterMissionStats(chapter.id);
    const stars = stats.done >= 20 ? 3 : stats.done >= 14 ? 2 : stats.done >= 7 ? 1 : 0;
    document.getElementById('ms-stars-badge').textContent = `${'★'.repeat(stars)}${'☆'.repeat(3-stars)} ${stats.done}/20`;

    // XP Level bar
    if (levelInfo) {
      document.getElementById('ms-level-num').textContent = `Lv. ${levelInfo.level}`;
      document.getElementById('ms-level-xp').textContent  = `${levelInfo.xpIntoLevel} / ${levelInfo.xpForNextLevel} XP`;
      document.getElementById('ms-xp-fill').style.width   = `${Math.round(levelInfo.progress01 * 100)}%`;
    }
    ...
    document.getElementById('ms-easy-done').textContent  = `${easyDone}/7`;
    document.getElementById('ms-med-done').textContent   = `${medDone}/7`;
    document.getElementById('ms-hard-done').textContent  = `${hardDone}/6`;
    document.getElementById('ms-total-done').textContent = `${stats.done}/20`;
    ...
    missions.forEach((m, i) => {
      ...
      const pct      = done ? 100 : Math.min(99, Math.round((progress / m.target) * 100));

      const card = document.createElement('div');
      card.className = `ms-mission-card diff-${m.difficulty}${done ? ' completed' : ''}`;
      card.dataset.missionId = m.id;

      card.innerHTML = `
        <div class="ms-mission-icon">${m.icon}</div>
        <div class="ms-mission-body">
          <div style="display:flex;align-items:center;gap:5px;margin-bottom:2px;">
            <div class="ms-mission-label">${m.label}</div>
            <span class="ms-diff-badge ${m.difficulty}">${m.difficulty}</span>
          </div>
          <div class="ms-mission-desc">${m.desc}</div>
          <div class="ms-mission-prog-bar">
            <div class="ms-mission-prog-fill" style="width:${pct}%"></div>
          </div>
          <div class="ms-mission-rewards">
            <span class="ms-reward-pill">✨ ${m.xpReward} XP</span>
            <span class="ms-reward-pill">🪙 ${m.coinReward}</span>
            ${done ? '<span class="ms-reward-pill" style="color:#4CAF50;border-color:rgba(76,175,80,0.4)">DONE</span>' : `<span class="ms-reward-pill" style="opacity:0.5">${pct}%</span>`}
          </div>
        </div>
      `;
      ...
      if (!done) {
        card.addEventListener('click', () => {
          // Deselect all
          list.querySelectorAll('.ms-mission-card').forEach(c => c.classList.remove('selected'));
          card.classList.add('selected');
          _selectedMission = m;

          document.getElementById('ms-selected-info').textContent = `Selected: ${m.icon} ${m.label}`;
    ...
    document.getElementById('ms-selected-info').textContent = 'Tap a mission to select';

    showScreen('mission-select-screen');
  });
}
