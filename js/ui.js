...
export class UI {
  constructor({ onRestart, onUsePower, onPhoenixContinue } = {}) {
    ...
    this.phoenixOverlayEl = document.getElementById('phoenix-save-overlay');
    this.phoenixContinueBtn = document.getElementById('phoenix-save-continue-btn');
    this.phoenixContinueBtn?.addEventListener('click', () => {
      playClickSound();
      this.phoenixOverlayEl?.classList.remove('visible');
      onPhoenixContinue?.();
    });
    ...
    this.goRestoreMsgEl   = document.getElementById('go-restore-msg');
  ...
  // Shows the unmissable, must-acknowledge Phoenix-save overlay. The
  // caller (Game) is expected to pause while this is up, and resume only
  // once the player taps "Continue Playing" (wired via onPhoenixContinue
  // in the constructor) — this guarantees the moment can never be missed
  // or mistaken for a silent restart.
  showPhoenixSave() {
    this.phoenixOverlayEl?.classList.add('visible');
  }

  // Early-warning cue: as the stack's peak climbs toward the danger line
  // (before anything has actually settled there), gently glow the stage's
  // outer edge so the player gets advance notice that things are getting
  // risky — rather than only finding out once a fruit is already parked
  // on the line and the fail-clock has started.
  setApproachGlow(tension01) {
    if (!this.stageEl) this.stageEl = document.querySelector('.stage');
    if (this.stageEl) {
      this.stageEl.classList.toggle('approach-warning', tension01 > 0.12);
      this.stageEl.style.setProperty('--approach-tension', String(Math.min(1, tension01)));
    }
  }
  ...
  showGameOver(finalScore, sessionStats = {}) {
    ...
    // Orchard restoration encouragement message
    if (this.goRestoreMsgEl) {
      const merges = sessionStats.sessionMerges ?? 0;
      const xp = sessionStats.sessionXP ?? 0;
      let msg = '';
      if (merges >= 50) {
        msg = 'The orchard stirs with new life! Keep restoring…';
      } else if (merges >= 20) {
        msg = 'Life Energy flows back into the land. You are the Keeper.';
      } else if (merges >= 5) {
        msg = 'The first seeds of restoration are planted.';
      } else if (xp > 0) {
        msg = 'Every merge brings the Eternal Orchard closer to life.';
      }
      this.goRestoreMsgEl.textContent = msg;
    }
  ...
  _renderXP(info) {
    ...
    if (this.xpFractionLabelEl) this.xpFractionLabelEl.textContent = `${info.xpIntoLevel} / ${info.xpForNextLevel} Life Energy`;
  }
  ...
  _initCoinsDisplay() {
    ...
    if (totalCoinsEl) totalCoinsEl.textContent = `Coins: ${getTotalCoins()}`;
    onCoinsChange((total) => {
      if (totalCoinsEl) totalCoinsEl.textContent = `Coins: ${total}`;
    });
  }
  ...
  updatePowerButtons() {
    ...
    for (const def of POWER_DEFS) {
      ...
      btn.innerHTML = `<span class="power-name">${def.name}</span><span class="power-count">${count}</span>`;
